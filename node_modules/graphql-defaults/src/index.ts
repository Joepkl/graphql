import {
  ...
  SelectionSetNode, ObjectTypeDefinitionNode,
...
import deepFreeze from 'freeze-object';
import * as dot from 'dot-wild';
import _ from 'lodash';
import { SchemaDefinitionNode } from 'graphql/language/ast';

const { forEach, has, get } = dot;
const { isEmpty, isNil, merge, cloneDeep, isEqual, isObject, set, camelCase } = _;
...
const TypesMaps: { [key: string]: any } = {};
let Schema: any;
let SchemaByTypeName: any;
...
export const graphQLDefaultsVueMixin: iGraphQLDefaultsVueMixin = {
...
export function mergeWithDefaults ({
  ...
  if (!defaults) {
    ...
    if (!defaults && !!operation) {
      ...
      if (!!path) defaults = _.get(defaults, path, defaults);
    }
  ...
  while (has(defaults, wPath)) {
    ...
    forEach(dataTmp, wPath,
        ...
        (_val, key, ctx, cPath) => {
          ...
          let _default = _.get(defaults, _defaultPath);
          if (debug && _.isUndefined(_default)) {
 ...
 * @return {*}
 */
function assign (_default: any, v: any): any {
 ...
 */
// @ts-ignore todo
export function isDefault (path: string, context = this) {
 ...
 */
// @ts-ignore todo
export function getDefault (path?: string | object, context = this) {
  if (isObject(path)) {
    context = path;
    path = undefined;
  }
 ...
 */
// @ts-ignore todo
export function hasDefault (path: string, context = this) {
  if (!!path) {
 ...
 */
// @ts-ignore todo
export function resetToDefaults (path?: string | object, context = this) {
  if (isObject(path)) {
    context = path;
    path = undefined;
  }
 ...
 * @return {boolean}
 */
function matchExclusion (def: any, exclude: any[string]) {
  return exclude.some((ex: string) => def.name.value.match(new RegExp(ex)));
}
 ...
 */
export function genTypesDefinitionsMaps (schema: DocumentNode | string, exclude = []) {
  ...
  if (!Array.isArray(schema.definitions) || !_.get(schema, 'definitions.length')) {
  ...
  for (let i = 0; i < Schema.definitions.length; i++) {
    ...
    if (matchExclusion(def, exclude)) continue;
    ...
    if (
      ...
      for (let fi = 0; fi < def.fields?.length; fi++) {
        ...
        if (matchExclusion(field, exclude)) continue;
      ...
      if (!_.has(defaults, def.name.value)) {
        ...
        // @ts-ignore todo
        defaults[def.name.value] = _default || !!kind && !!defaults[kind] ? defaults[kind] : '';
      }
  ...
  console.debug(`[index/genTypesDefinitionsMaps()]-619: defaults: %o`, defaults);
  ...
  for (const type in TypesMaps) {
    ...
    if (_.isEmpty(typeMap.fields) && _.isUndefined(typeMap.types)) {
      // @ts-ignore
      const default_ = defaults[typeMap.type];
      ...
      typeMap.fields = typeMapWithFields?.fields;
    }
  }
 ...
 */
function addDefToTypesMaps (def: ObjectTypeDefinitionNode, exclude: string[]) {
  /** make fields */
  const fields = (def.fields ?? []).reduce((accum: object, field: FieldDefinitionNode) => {
    if (matchExclusion(field, exclude)) return accum;
    
    if (TypesMaps[def.name.value]) throw new Error(`[graphql-defaults/addDefToTypesMaps()]-527: TypeMap: "${def.name.value}" was already defined!`);
    
    let fieldTypeKind = getTypeName(field);
    
    const type = fieldTypeKind;
    let isScalar = _.has(defaults, fieldTypeKind);
    ...
    // @ts-ignore todo
    let _default = defaults[fieldTypeKind];
  ...
  TypesMaps[def.name.value] = {
    type: getTypeName(def),
    // @ts-ignore
    ...(!_.isEmpty(def.types) ? { types: def.types } : {}),
    // @ts-ignore
    kind: def.kind === 'EnumTypeDefinition' ? 'String' : getTypeMapKind(def),
 ...
 * @returns {*}
 */
export function genGraphQLDefaults ({
                                      operation,
                                      // @ts-ignore todo
                                      context = this,
                                      path,
                                      populateListTypes = true,
                                      genTypename = true,
                                      debug = GraphqlDefaults.debug,
                                    }: iGenGraphQLDefaultsOpts): object {
 ...
 * @param value
 */
function setContextDefault (context: object, path?: string, value?: object) {
  // @ts-ignore todo
  let _path = path || _.get(Object.keys(value), '[0]');
  const _value = deepFreeze(cloneDeep(_.get(value, _path)));
  
  if (_.get(context, '_isVue')) {
    _path = `$gqlDefaults.${_path}`;
  }
  
  _.set(context, _path, _value);
 ...
 */
function getObjLeaf (initObj: object, step: string, leafPath: string, failPath?: string) {
  let path = step;
  let obj, leaf;
  ...
  if (!leaf) return get(initObj, failPath ?? leafPath);
 ...
 * @returns {*}
 */
function getTypeName (rootType: object) {
 ...
 * @returns {*}
 */
function getTypeKind (rootType: object) {
  return getObjLeaf(rootType, '.type', 'kind');
}


// @ts-ignore todo
function getTypeMapKind (typeMap: any) {
  return _.get(typeMap, 'type.kind', _.get(typeMap, 'kind')) === 'ListType' ? 'ListType' : 'Object';
}
 ...
 */
// todo
function genFragmentsMaps (defs: DefinitionNode[]) {
  let fragmentsMaps = {};
  
  defs.filter((qd: DefinitionNode) => qd.kind === 'FragmentDefinition').forEach((f: DefinitionNode) =>
      // @ts-ignore todo
 ...
 * @param options
 */
function parseSetSelections (selections: any, parentTypeMap: any, fragmentsMaps: any, options: any) {
  let selsObj = {};
...
[graphql-defaults]-641: parseSetSelections() - selectionName: %o, selection: %o`, selectionName, selection);
    ...
    if (selection.kind === 'FragmentSpread') {
      ...
      if (!fragment) throw new Error(`[graphql-defaults]-647: GraphQL-Defaults: Error parsing fragment spread - Missing fragment: "${selectionName}" - Please make sure you import/include the specified missing fragment in the query being processed!`);
      
      // @ts-ignore todo
      let tMap = TypesMaps[fragment.type];
      selsObj = {
        ...parseSetSelections(fragment.selections, tMap, fragmentsMaps, options),
      };
      
      // @ts-ignore todo
      if (options.genTypename && !isEmpty(selsObj)) selsObj.__typename = fragment.type;
      ...
      if (!typeMap) {
        ...
        // @ts-ignore todo
        if (options.genTypename && !isEmpty(parseResult)) parseResult.__typename = typeMap.type;
...
// @ts-ignore todo
function getParentTypeMapSelection (selName: string, typeMap: any) {
  // console.debug(`[graphql-defaults]-722: getParentTypeMapSelectionKind() -   typeMap: %o`, typeMap);
  return get(typeMap, `fields.${selName}`);
}

// @ts-ignore todo
function getParentTypeMapSelectionKind (selName: string, typeMap: any) {
